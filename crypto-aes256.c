/*
  AES-256 for prototyping

  This is a simple AES implementation for prototyping purposes,
  so that new projects don't need to deal with complex dependencies.
  It's both slow and unsafe (vulnerable to side channel attacks).
 ...
 */
static size_t hex_decode(const char *src, unsigned char *dst, size_t dst_max) {
    size_t i;
    size_t dst_offset = 0;
...
int AES256_selftest(int is_verbose) {
    uint8_t key[32];
    uint8_t out[64];
    uint8_t iv[16];
    uint8_t in[64];
    ...
    hex_decode("234b96b5349e39f221481eb91b25ef20"
               "a2a93b68b37eb5785b51aadda36150db",
               key, sizeof(key));
    hex_decode("0242dca8280f7408d5f45f91540d0baa"
               "0ed8fbe1dc725b9e79945fe3aab39639"
               "78efe8d97089704c3e09ebff902023de"
               "44553177293733bacaff00f47545b180",
               out, sizeof(out));
    hex_decode("4367627b7897b3e4efaef9a38cb49611", iv, sizeof(iv));
    hex_decode("000000206674797069736f6d00000200"
    ...
    if (0 == memcmp(out, in, 64)) {
        if (is_verbose)
            fprintf(stderr, "[+] AES-256: selftest success\n");
        return 0;
    } else {
        if (is_verbose)
            fprintf(stderr, "[-] AES-256: selftest failure\n");
        return 1;
    }
}
